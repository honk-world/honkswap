...
describe("MasterChef", function () {
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should have a balance of 25000000000 HONK", async function() {
      ...
      await this.sushi.mint(this.chef.address, 25000000000);

      expect(await this.sushi.balanceOf(this.chef.address)).to.equal(25000000000);
    })
    ...
    it("should give out SUSHIs only after farming time", async function () {
      ...
      expect(await this.sushi.totalSupply()).to.equal("25000000000")
    })
    ...
    it("should distribute SUSHIs properly for each staker", async function () {
      ...
      expect(await this.sushi.balanceOf(this.chef.address)).to.equal("24999994334")
      // expect(await this.sushi.balanceOf(this.dev.address)).to.equal("1000")
      ...
      expect(await this.sushi.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.sushi.balanceOf(this.chef.address)).to.equal("24999988144") //8144
    ...
    // it("should increase the rewards appropriately", async function () {
    //   // variable farming rate based on the block since created
    //   this.chef = await this.MasterChef.deploy(this.sushi.address, this.dev.address, "0", "0", "0")

    //   // Create LP and add 100 tokens
    //   await this.lp.connect(this.alice).approve(this.chef.address, "100", { from: this.alice.address })
